(() => {
  ...
  async function getCountryInfo(country) {
    ...
    try {
      ...
      result.innerHTML = `\n    <div class="result__image"><img src="${
        data[0].flags.svg
      }" alt="flag" /></div>\n    <h2 class="result__country">${
        data[0].name.common
      }</h2>\n    <ul class="result__info">\n      <li class="info__item"><span>Столица:</span> ${
        data[0].capital[0]
      }</li>\n      <li class="info__item"><span>Континент:</span> ${
        data[0].continents[0]
      }</li>\n      <li class="info__item"><span>Население:</span> ${
        data[0].population
      }</li>\n      <li class="info__item"><span>Валюта:</span> ${
      ...
      if (country.length == 0) result.innerHTML = `<h3>Поле ввода не может быть пустым!</h3>`;
      else result.innerHTML = `<h3>Ошибка! Страна не найдена!.</h3>`;
    }
  }
  function start() {
